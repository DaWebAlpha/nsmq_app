...
/**
 * Lists every note (active and deleted) created by one user, paginated.
 * @param {object} params
 ...
 * @throws {NotFoundError} If this user has no notes at all.
 */
const getAllNotesCreatedByUserService = async({
    userId,
    sortOrder = -1,
} = {}) => {
    ...
    const notes = await Note.paginate({
        filter: {userId, isDeleted: {$in: [true, false]}},
        options: {sort: {createdAt: sortOrder}, lean: true},
    })

    if(notes.data.length === 0){
        throw new NotFoundError({
            message: "No notes exists yet",
...
export {
    getAllNotesCreatedByUserService,
}